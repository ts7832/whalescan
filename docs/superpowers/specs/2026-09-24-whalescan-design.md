...
| Data API — closed positions | `GET https://data-api.polymarket.com/closed-positions?user=&limit&offset&sortBy=TIMESTAMP&sortDirection=DESC` | resolved positions: `avgPrice`, `totalBought`, `realizedPnl`, `curPrice` (1/0 at resolution), `conditionId`, `outcome`, `timestamp`. **Default sort is `realizedPnl` descending** (verified: page 1 of a top wallet = 100% winners, ascending sort = 100% losers). We always sort by `TIMESTAMP` and paginate to exhaustion. If the API's 10,000-row depth limit cuts a very active wallet's history, what remains is its most recent contiguous window — time-ordered, so not biased toward winners — and it is scored as such (marked `truncated`). A fetch refused before any page arrives is `incomplete` and the wallet is not testable. |
| Data API — positions | `GET https://data-api.polymarket.com/positions?user=&sizeThreshold=0&limit=500&offset` | resolved-but-unredeemed positions (`redeemable: true`). **Required:** losing positions are rarely redeemed (they pay $0), so they never appear in `/closed-positions`; scoring from closed positions alone is almost pure survivorship bias (verified: 97% win rate at average price 0.50 on one top wallet). Every wallet history is the union of both endpoints. |
...
1. **Universe**: union of leaderboard wallets (several windows) and every wallet with a ≥ $5,000 trade in the last 30 days; capped at `max_wallets` (default 3,000: leaderboard wallets first, then large-trade wallets by volume).
2. **Positions**: fetch *all* closed positions per wallet (see Part 2 sort caveat). A position counts only if its market is resolved in Gamma (`closed = true` and `outcomePrices` exactly `["1","0"]` or `["0","1"]`), which gives `y ∈ {0, 1}`. Markets resolved fractionally (50/50 splits, disputed/voided) are **discarded** — the no-skill null `y* ~ Bernoulli(p)` cannot produce fractional outcomes, so mixing them in would bias the test. Positions in markets that are still open (the wallet sold early) are excluded. Keep entry `avgPrice ∈ [0.03, 0.97]`, non-blocklisted markets. Stake `w = totalBought · avgPrice`, **winsorized** at the wallet's 95th percentile so one giant bet can't dominate.
...
- **Python (pytest):** parsers against recorded real JSON fixtures in `tests/fixtures/`; closed-positions pagination fetches every page with `sortBy=TIMESTAMP`; a depth-capped history keeps its recent window, and both sources (closed + unredeemed) are cut to that window before scoring; fractional/unresolved markets are excluded; BH against a hand-computed example; shrinkage math; classifier flags; every gate G1–G7 with pass/fail cases; position-event aggregation windows; `ReconnectingWS` against a local test WS server that drops connections; end-to-end `batch` on fixtures producing snapshot JSON.
